...
const GET_BY_USER_ID_QUERY: string = `SELECT * FROM \`${DATASET_ID}.${USER_TABLE}\` WHERE userId = @userId LIMIT 1`
...
export const updateUserTeamId = async (
  ...
  const user = await getUserById(andrewId)
  if (user.teamId) {
    return undefined
  }
